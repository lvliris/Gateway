...
user_esp_platform_device_action(struct wait_param *pwait_action)
{
    ...
    TM_DEBUG("%d device action\n", action_number);

#if PLUG_DEVICE
    for (i = 0; i < action_number && pwait_action->action[i][0] != '0'; i++) {
        TM_DEBUG("%s\n",pwait_action->action[i]);
        if(strncmp(pwait_action->action[i], "mode", 4) == 0)
        {
        	uint8* TimerModeIdx = (uint8*)zalloc(sizeof(uint8));
        	*TimerModeIdx = atoi(pwait_action->action[i] + 4);
        	TM_DEBUG("excute mode %d\n", *TimerModeIdx);
        	portBASE_TYPE xStatus;
        	xStatus = xTaskCreate(SceneOrder, "SceneOrder", 512, TimerModeIdx, 2, NULL);
        	if(xStatus == pdTRUE)
        		TM_DEBUG("CREATE TASK SUCCESS\n");
        	else
        		TM_DEBUG("FAILED TO ALLOCATE ENOUGH ROOM\n");
			//WaitForSendOrderReadyISR();
        }
