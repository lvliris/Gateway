...
struct client_conn_param *mclient_param = NULL;
...
user_esp_platform_set_token(uint8_t *token)
{
    ...
    //ESP_DBG("esp_param.tokenrdy:%d",esp_param.tokenrdy);
    
    if(strlen(token)<=40)
    {
        memcpy(esp_param.token, token, strlen(token));
        memcpy(esp_param.devkey, token, strlen(token));
    }
    else
...
//            ESP_DBG("get complete timer info\n");
//            bool *humaniz_format = (bool*)zalloc(sizeof(bool));
//            *humaniz_format = false;
//            portBASE_TYPE xStatus;
//            xStatus = xTaskCreate(user_master_timer_get_task, "user_master_timer_task", 176, NULL, 2, NULL);
//            if(xStatus == pdTRUE)
//            	ESP_DBG("create timer get task succeess\n");
//            else
//            	ESP_DBG("create timer get task failed\n");
            ...
            user_master_timer_index_update(pusrdata);
        }
            ...
            *humaniz_format = false;
            ...
            //xStatus = xTaskCreate(user_master_timer_get_task, "user_master_timer_task", 176, humaniz_format, 2, NULL);
            if(xStatus == pdTRUE)
            	ESP_DBG("create timer get task succeess\n");
            else
            	ESP_DBG("create timer get task failed\n");
...
user_esp_platform_maintainer(void *pvParameters)
{
    ...
    if( REBOOT_MAGIC != user_boot_flag && 0 == user_get_key_status()){
    	...
    	ESP_DBG("enter softap+station mode\n");
#if (PLUG_DEVICE || SENSOR_DEVICE)
        ...
        if(1){/*for test*/
            ...
            sprintf(sta_config->ssid, "zhaofengkeji");
            sprintf(sta_config->password, "88888888");
        ...
        free(sta_config5);
        ESP_DBG("wifi_station_get_ap num %d\n",ret);
        if(0 == ret) {
    ...
    while(1){
        ...
        ESP_DBG("iot_pltaform socket %d\n",client_param.sock_fd);
        ...
        do{
            ...
            if(0 != ret){
                ...
                //copy the client parameter for user_master.c
                mclient_param = &client_param;
                break;
            }
        ...
        user_esp_platform_sent(&client_param);//step3
        ...
        while(1){
            ...
            if ((ret) > 0){
            	...
            	//ESP_DBG("ret=%d sock_fd=%d\n",ret,client_param.sock_fd);
            	if(ret == -1)//it seems to lack a condition here
            	{
            		ESP_DBG("socket error\n");
            		esp_param.activeflag = 0;
                    system_param_save_with_protect(ESP_PARAM_START_SEC, &esp_param, sizeof(esp_param));
                    user_esp_platform_discon(&client_param);
            		break;
                ...
                if((sta_ipconfig.ip.addr == 0 || wifi_station_get_connect_status() != STATION_GOT_IP)){
                	ESP_DBG("lose connection to router\n");
                    user_esp_platform_discon(&client_param);
                    timeout_count = 0;
                    break;
                }
                ...
                if(timeout_count++ > BEACON_TIME/nNetTimeout){
                    if (ping_status == FALSE) {        //disconnect,exit the recv loop,to connect again
                        ESP_DBG("user_esp_platform_sent_beacon,server no response, and beacon time comes again!\n");
                        user_esp_platform_discon(&client_param);
                        break;
                    }
