...
LOCAL void long_press(void){
	os_printf("long press\n");
	GPIO_OUTPUT(GPIO_Pin_5, 1);
	spi_flash_erase_sector(0x104);
	system_restart();
}
LOCAL void short_press(void){
...
user_plug_init(void)
{
    ...
    single_key[1] = key_init_single(GPIO_ID_PIN(2), GPIO_PIN_REG_2, FUNC_GPIO2, long_press, short_press);
