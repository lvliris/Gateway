...
bool ping_status = true;
LOCAL xQueueHandle	Web_QueueStop = NULL;
xQueueHandle MasterIdReady = NULL;
const char *ping_msg =  "{\"path\": \"/v1/ping/\", \"method\": \"GET\"}";

#define local_host_name		"101.201.211.87"//"192.168.0.200"//"iot.espressif.cn"//"121.40.165.18"//
#define local_host_url		"/zfzn02/websocket_master/"//"zfzn02/servlet/ElectricOrderServlet"//"v1/datastreams/tem_hum/datapoint"//"/1"//
#define local_host_port		"8080"//"9000"//
#define local_host_ports	"9443"//"8080"//

#define WEBSOCKET_PING_TIME 10
...
nopoll_bool test_02 (void) {
	...
	ctx = create_ctx ();
	if(!ctx)
		nopoll_log (ctx, NOPOLL_LEVEL_DEBUG, "ctx null", host);
	/* call to create a connection */
	conn = nopoll_conn_new (ctx, local_host_name, local_host_port, NULL, local_host_url, NULL, NULL);
	...
	/* send content text(utf-8) */
	if (nopoll_conn_send_text (conn, ping_msg, strlen(ping_msg)) != strlen(ping_msg)) {//"This is a test"
		printf ("ERROR: Expected to find proper send operation..\n");
		return nopoll_false;
	}
...
LOCAL int websocket_main (char *argv)
{
	...
	switch (iterator) {
		...
		case 3:
			if (test_03()) {
				printf("Test 03: Simple request/reply [   OK   ]\n");
			} else {
				printf("Test 03: Simple request/reply [ FAILED ]\n");
			}
...
void websocket_heartbeat(void *parameters)
{
	noPollConn *conn = (noPollConn*)parameters;
	bool stopStatus = true;
	while(1){
		vTaskDelay(6000/ portTICK_RATE_MS);
//		if(!nopoll_conn_is_ready(conn))
//			continue;
		WEBSOCKET_DBG("websocket heartbeat task ping...\n");
		if(nopoll_conn_send_text(conn, "websocket ping\n", strlen("websocket ping\n")) != strlen("websocket ping\n"))
		{
			WEBSOCKET_DBG("websocket heartbeat task ping failed\n");
			nopoll_conn_shutdown(conn);
			xQueueSend(Web_QueueStop, &stopStatus, 0);
			continue;
		}
		ping_status = false;
		vTaskDelay(6000/ portTICK_RATE_MS);
		...
		{
			WEBSOCKET_DBG("websocket heartbeat task ping fail\n");
			nopoll_conn_shutdown(conn);
			//xQueueSend(Web_QueueStop, &stopStatus, 0);
			continue;
		}
...
LOCAL int websocket_run()
{
	noPollCtx *ctx;
	noPollMsg *msg;
	uint8 order[256];
	uint8 ping_timeout = 0;
	xTaskHandle heartHandle;
	WEBSOCKET_DBG("WEBSOCKET running!\n");
	uint8 websocketURL[40] = { 0 };
	while(!ctrlid[0])
	{
	...
	UploadVersion();
	sprintf(websocketURL, "/zfzn02/websocket_master/%s", ctrlid);
	WEBSOCKET_DBG("websocket url:%s\n", websocketURL);
	ctx = create_ctx();
	if(nopoll_ctx_conns(ctx) != 0)
	{
		printf ("ERROR: expected to find 0 registered connections but found: %d\n", nopoll_ctx_conns (ctx));
	...
	/* call to create a connection */
	conn = nopoll_conn_new (ctx, REMOTE_IP, local_host_port, NULL, websocketURL, NULL, NULL);
	if (! nopoll_conn_is_ok (conn)) {
		printf ("ERROR: Expected to find proper client connection status, but found error.. (conn=%p, conn->session=%d, NOPOLL_INVALID_SOCKET=%d, errno=%d, strerr=%s)..\n",
			conn, (int) nopoll_conn_socket (conn), (int) NOPOLL_INVALID_SOCKET, errno, strerror (errno));
		return nopoll_false;
	...
	{
		printf("wait connection ready timeout\n");
		return nopoll_false;
	}

	WEBSOCKET_DBG("create websocket heartbeat task\n");
	//xTaskCreate(websocket_heartbeat, "websocket_heartbeat", 512, conn, 2, &heartHandle);
	WEBSOCKET_DBG("sending the first message...\n");

	/* send content text(utf-8) */
//	if (nopoll_conn_send_text (conn, "\"version\":\"v1.0.5\"",strlen("\"version\":\"v1.0.5\"")) != strlen("\"version\":\"v1.0.5\"")) {
//		printf ("ERROR: Expected to find proper send operation..\n");
//		return nopoll_false;
//	}

	vTaskDelay(1000 / portTICK_RATE_MS);
	nopoll_conn_send_text(conn, "websocket ping\n", strlen("websocket ping\n"));
...
//		vTaskDelay(1000 / portTICK_RATE_MS);
//		WEBSOCKET_DBG("delete websocket heartbeat task\n");
//		vTaskDelete(heartHandle);

		portBASE_TYPE xStatus;
		bool ValueFromReceive;
		...
		memset(order, 0, 256);
/* 		int recvbytes = nopoll_conn_read(conn, order, 256, false, 3000);
		if(recvbytes <  0)
		{
			WEBSOCKET_DBG("ERROR: read data fail!\n");
			break;
		}
		else if(recvbytes == 0)
		{
			WEBSOCKET_DBG("Receive no data\n");
		}
		else
		{
			ProcessOrder(order, recvbytes);
			WEBSOCKET_DBG("Receive data:%s\n", order);
		} */

		msg = nopoll_conn_get_msg (conn);
		if(msg == NULL)
		{
			nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "ERROR: received websocket connection close during wait reply..\n");
			WEBSOCKET_DBG("no message received!\n");
			break;
		}
		else
		{
			memcpy(order, (uint8*)nopoll_msg_get_payload (msg), msg->payload_size);
			if(ping_status == false)
			{
				if(strstr(order, "ping") != NULL)
				{
					WEBSOCKET_DBG("websocket ping success\n");
					//if(testTimes++ < 10)
						ping_status = true;
				}
				else
				{
					WEBSOCKET_DBG("websocket ping fail\n");
					nopoll_msg_unref(msg);
					nopoll_conn_shutdown(conn);
					break;
				}
			}
			ProcessOrder(order, msg->payload_size);
			WEBSOCKET_DBG("Recieve data:%s\n",(const char *) nopoll_msg_get_payload (msg));
		}
...
//		if(!nopoll_conn_is_ok (conn))
//			break;
//		if(nopoll_conn_send_text (conn, "websocket test\n",strlen("websocket test\n")) != strlen("websocket test\n"))
//		{
//			printf("send fail\n");
//			break;
//		}
...
void websocket_start(void *optarg)
{
	...
	if(MasterIdReady == NULL)
		MasterIdReady = xQueueCreate(1,1);

	if (Web_QueueStop != NULL && MasterIdReady != NULL)
		xTaskCreate(websocket_task, "websocket_task", 1024, optarg, 2, NULL);
}
